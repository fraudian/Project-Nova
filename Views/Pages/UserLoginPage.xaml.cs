...
namespace NovaLauncher.Views.Pages
{
    ...
    public partial class UserLoginPage : Page
    {
        ...
        private async Task LoginAsync()
        {
            ...
            if (!string.IsNullOrWhiteSpace(emailBox.Text) && !string.IsNullOrWhiteSpace(txtPassword.Password))
            {
                Logger.Log(LogLevel.Info, "Login successful.");

                // Simulate successful login
                AccountUtils.MyLauncherAccount = new Account(); // Replace with actual account if needed
                ...
                // Optional: Save user if needed
                if (bSaveUser) { AccountUtils.SaveNewUser(emailBox.Text, txtPassword.Password); }

                // Start host service and close main window
                Application.Current.Dispatcher.Invoke(() =>
                {
                    HostService._host.StartAsync(); // Start your host service here
                    LauncherData.MainWindowView.Close(); // Close your main window
                });
            }
            else
            {
                // Fields are empty or invalid login attempt
                Logger.Log(LogLevel.Error, "Login failed.");

                LoginCard.Visibility = Visibility.Visible;
                snackBar.Timeout = 5000;
                snackBar.Show("Access Denied", "Invalid email or password.", SymbolRegular.LockClosed24, ControlAppearance.Danger);
            }
        }
